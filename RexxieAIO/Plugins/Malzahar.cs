...
namespace RexxieAIO.Plugins
{
    class Malzahar
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            Config.SubMenu("Combo").AddItem(new MenuItem("not01", "// = If Killable")).SetValue(true);
            ...
            Config.SubMenu("Misc").AddItem(new MenuItem("RInterrupt", "R to Interrupt").SetValue(false));
        ...
        private static float GetComboDamage(Obj_AI_Hero hero)
        {
            ...
            if (W.IsReady())
                damage += Hero.GetSpellDamage(hero, SpellSlot.W) * 1.75;

            if (E.IsReady() || hero.HasBuff("AlZaharMaleficVisions"))
        ...
        private static void On_InterrupteptableTarget(Obj_AI_Hero sender, Interrupter2.InterruptableTargetEventArgs args)
        {
            if (Config.Item("RInterrupt").GetValue<bool>())
                CastOnUnit(R, sender, HitChance.High);
        ...
        private static void OnGameUpdate(EventArgs argum)
        {
            ...
            if (Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Combo)
            {
                ...
                if (Config.Item("UseRCombo").GetValue<bool>() && GetComboDamage(target) > target.Health)
                    CastOnUnit(R, target, HitChance.High);        
                ...
                if (Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.LaneClear)
                {
                    ...
                    if (minions.Count > 1)
                    {
                        ...
                        if (Config.Item("UseWWave").GetValue<bool>())
                            CastSpell(W, minion);
                        if (Config.Item("UseEWave").GetValue<bool>())
                            CastOnUnit(E, minion, HitChance.High);
                    }
                }
